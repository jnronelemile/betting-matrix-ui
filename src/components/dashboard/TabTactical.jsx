    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="pt-2 border-t border-slate-800">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {Object.entries(match.Asian_Handicaps).filter(([k]) => k.startsWith('AH_')).map(([k, v]) => (
                    <div key={k} className="bg-slate-900 border border-slate-800 p-3 rounded-lg flex justify-between items-center shadow-inner">
                      <span className="text-[10px] text-slate-500 uppercase tracking-widest">{k.replace('_', ' ')}</span>
      ...
      <div className="lg:col-span-1">
        <Card title="Match vs Moyennes Ligue" titleIcon={<Layers size={16} />}>
          <div className="space-y-4 mt-2 overflow-x-auto custom-scrollbar pb-2">
            <div className="min-w-[340px]">
              ...
              {anchorKeys.map(({ key, label, prop, lambda }) => {
                ...
                return (
                  <div key={key} className="flex items-center justify-between bg-slate-950 p-4 rounded-xl border border-slate-800 hover:border-slate-700 transition-colors mt-4">
