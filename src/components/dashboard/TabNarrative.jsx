...
export default function TabNarrative({ match }) {
  ...
  const getObjectiveColor = (obj) => {
    if (obj?.includes('TITLE')) return 'text-amber-600 dark:text-amber-400';
    if (obj?.includes('EUROPE')) return 'text-blue-600 dark:text-blue-400';
    if (obj?.includes('SURVIVAL')) return 'text-rose-600 dark:text-rose-400';
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      {/* Team Profiles */}
      <Card title="Situational Profiles" titleIcon={<Brain size={16} />}>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {/* Home Team */}
          <div className="bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 p-5 relative overflow-hidden shadow-sm">
            <div className="absolute top-0 left-0 w-1 h-full bg-blue-500"></div>
            <h4 className="text-xs uppercase tracking-widest text-slate-500 font-bold mb-4">Home Team</h4>
            
            <div className="space-y-4">
              <div>
                <div className="text-[10px] text-slate-500 uppercase tracking-widest mb-1">Current Standing</div>
                <div className="flex items-baseline gap-2">
                  <div className="font-mono text-xl text-slate-800 dark:text-slate-200">#{homePerf.actual_league_position || '?'} <span className="text-[10px] text-slate-500 font-sans tracking-widest uppercase">Overall</span></div>
                  <div className="font-mono text-sm text-slate-500 dark:text-slate-400">#{home.rank_home || '?'} <span className="text-[10px] font-sans tracking-widest uppercase">Home</span></div>
                </div>
              </div>
              
              <div>
                <div className="text-[10px] text-slate-500 uppercase tracking-widest mb-1">Objective</div>
                <div className={`text-xs font-bold uppercase tracking-wider ${getObjectiveColor(home.objective)}`}>
                  {home.objective?.replace(/_/g, ' ') || 'UNKNOWN'}
                </div>
              </div>

              <div>
                <div className="text-[10px] text-slate-500 uppercase tracking-widest mb-1">Justice Status</div>
                <Badge variant={getJusticeVariant(home.justice_status)}>{home.justice_status || 'UNKNOWN'}</Badge>
              </div>

              <div>
                <div className="text-[10px] text-slate-500 uppercase tracking-widest mb-1">Motivation Index</div>
                <div className="flex items-center gap-2">
                  <div className="w-full bg-slate-200 dark:bg-slate-800 rounded-full h-1.5 overflow-hidden">
                    <div className="bg-emerald-500 dark:bg-emerald-400 h-full rounded-full" style={{ width: `${Math.min((psychology.motivation_home || 0) * 100, 100)}%` }}></div>
                  </div>
                  <span className="font-mono text-xs text-slate-600 dark:text-slate-300">{(psychology.motivation_home || 0).toFixed(3)}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 p-5 relative overflow-hidden shadow-sm">
            ...
            <h4 className="text-xs uppercase tracking-widest text-slate-500 font-bold mb-4 text-right">Away Team</h4>
            
            <div className="space-y-4 text-right">
              <div>
                <div className="text-[10px] text-slate-500 uppercase tracking-widest mb-1">Current Standing</div>
                <div className="flex items-baseline justify-end gap-2">
                  <div className="font-mono text-sm text-slate-500 dark:text-slate-400">#{away.rank_away || '?'} <span className="text-[10px] font-sans tracking-widest uppercase">Away</span></div>
                  <div className="font-mono text-xl text-slate-800 dark:text-slate-200">#{awayPerf.actual_league_position || '?'} <span className="text-[10px] text-slate-500 font-sans tracking-widest uppercase">Overall</span></div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className={`text-xs font-bold uppercase tracking-wider ${getObjectiveColor(away.objective)}`}>
                  {away.objective?.replace(/_/g, ' ') || 'UNKNOWN'}
                </div>
              </div>

              <div>
                <div className="text-[10px] text-slate-500 uppercase tracking-widest mb-1">Justice Status</div>
                <Badge variant={getJusticeVariant(away.justice_status)}>{away.justice_status || 'UNKNOWN'}</Badge>
              </div>

              <div>
                <div className="text-[10px] text-slate-500 uppercase tracking-widest mb-1">Motivation Index</div>
                <div className="flex items-center justify-end gap-2">
                  <span className="font-mono text-xs text-slate-600 dark:text-slate-300">{(psychology.motivation_away || 0).toFixed(3)}</span>
                  <div className="w-full bg-slate-200 dark:bg-slate-800 rounded-full h-1.5 overflow-hidden rotate-180">
                    <div className="bg-emerald-500 dark:bg-emerald-400 h-full rounded-full" style={{ width: `${Math.min((psychology.motivation_away || 0) * 100, 100)}%` }}></div>
                  </div>
                </div>
              </div>
            </div>
      ...
      {/* Performance Delta */}
      <Card title="Performance Delta (Expected vs Actual)" titleIcon={<LineChart size={16} />}>
        <div className="overflow-hidden rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 shadow-sm">
        ...
        <div className="mt-4 text-[10px] text-slate-500 dark:text-slate-500 uppercase tracking-widest flex gap-2 items-start bg-slate-100/50 dark:bg-slate-900/50 p-3 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm">
          ...
          <p>A positive delta in Points or Goals Scored means the team overperformed its expected metrics (lucky/clinical form). A negative delta means it underperformed (unlucky/undervalued).</p>
        </div>
      </Card>
    </div>
  );
