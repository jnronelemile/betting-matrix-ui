...
export default function TabCoreMarket({ match }) {
  ...
  const renderProbOdds = (prob, oddsValue) => (
    <span className="flex items-center justify-end gap-6 sm:gap-8 min-w-[140px]">
      <span className="text-slate-800 dark:text-slate-200 text-right w-12">{(prob * 100).toFixed(1)}%</span>
      <span className="text-emerald-600 dark:text-emerald-400 font-bold text-right w-16">@{oddsValue?.toFixed(2) || '-'}</span>
    </span>
  );
          ...
          <div className="space-y-6 mt-2">
            {isGoalsZero ? (
              ...
              <div className="flex flex-col gap-5">
                ...
                {/* Favorite Goals */}
                <div className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 p-3.5 rounded-xl shadow-sm relative overflow-hidden">
                  <div className="absolute top-0 left-0 w-1 h-full bg-sky-500/30"></div>
                  <div className="flex justify-between items-center mb-3 border-b border-slate-200 dark:border-slate-800/50 pb-2">
                    <span className="text-[10px] font-bold text-sky-600 dark:text-sky-400 uppercase tracking-wider">FAV: {teamGoals.Target_Entity?.Favorite}</span>
                  ...
                  <div className="flex flex-col gap-3">
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold tracking-tighter">Over 1.5</span>
                      <div className="flex gap-3">
                        <span className="text-xs font-bold text-slate-800 dark:text-slate-200">{(teamGoals.Favorite_Goals?.PROB_FAV_O15 * 100).toFixed(1)}%</span>
                        <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400 font-mono">@{teamGoals.Favorite_Goals?.ODDS_FAV_O15?.toFixed(2)}</span>
                      </div>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold tracking-tighter">Over 2.5</span>
                      <div className="flex gap-3">
                        <span className="text-xs font-bold text-slate-800 dark:text-slate-200">{(teamGoals.Favorite_Goals?.PROB_FAV_O25 * 100).toFixed(1)}%</span>
                        <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400 font-mono">@{teamGoals.Favorite_Goals?.ODDS_FAV_O25?.toFixed(2)}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 p-3.5 rounded-xl shadow-sm relative overflow-hidden">
                  ...
                  <div className="flex justify-between items-center mb-3 border-b border-slate-200 dark:border-slate-800/50 pb-2">
                    <span className="text-[10px] font-bold text-amber-600 dark:text-amber-400 uppercase tracking-wider">OUT: {teamGoals.Target_Entity?.Outsider}</span>
                  ...
                  <div className="flex flex-col gap-3">
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold tracking-tighter">Over 0.5</span>
                      <div className="flex gap-3">
                        <span className="text-xs font-bold text-slate-800 dark:text-slate-200">{(teamGoals.Outsider_Goals?.PROB_OUT_O05 * 100).toFixed(1)}%</span>
                        <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400 font-mono">@{teamGoals.Outsider_Goals?.ODDS_OUT_O05?.toFixed(2)}</span>
                      </div>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold tracking-tighter">Over 1.5</span>
                      <div className="flex gap-3">
                        <span className="text-xs font-bold text-slate-800 dark:text-slate-200">{(teamGoals.Outsider_Goals?.PROB_OUT_O15 * 100).toFixed(1)}%</span>
                        <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400 font-mono">@{teamGoals.Outsider_Goals?.ODDS_OUT_O15?.toFixed(2)}</span>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div className="flex flex-col gap-4 mt-2">
            ...
            {match.Super_Signals?.H_MINUS_25_ANNIHILATION && (
              <div className="bg-rose-500/10 border border-rose-500/30 p-3 rounded-xl flex gap-3 items-center shadow-sm">
                <Activity className="text-rose-600 dark:text-rose-400 shrink-0" size={16} />
                <span className="text-[10px] font-bold text-rose-600 dark:text-rose-400 tracking-widest uppercase">Annihilation H-2.5</span>
              </div>
            )}
            ...
            <div className="space-y-4 mt-1">
              ...
              <div className="grid grid-cols-1 gap-3">
                {Object.entries(match.Standard_Handicaps).filter(([k]) => k.startsWith('H_')).map(([k, v]) => (
                  <div key={k} className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 p-3 rounded-xl flex justify-between items-center shadow-sm">
                    <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-widest font-bold">{k.replace('_', ' ')}</span>
                    <div className="flex items-center gap-4">
                      <span className="text-xs font-bold text-slate-800 dark:text-slate-300">{(v.prob * 100).toFixed(1)}%</span>
                      <span className="font-mono text-xs text-emerald-600 dark:text-emerald-400 font-bold">@{v.odds?.toFixed(2)}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="flex flex-col gap-3 mt-1">
              {Object.values(scores).map((item, idx) => (
                <div key={idx} className="flex flex-wrap gap-2 justify-between items-center bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 p-3.5 rounded-xl shadow-sm">
                  <div className="flex items-center gap-4">
                    <span className="text-base font-bold text-slate-800 dark:text-slate-200 font-serif tracking-wider">{item.score}</span>
                    <Badge variant="default" className="text-[10px] px-2 py-0.5">{(item.prob * 100).toFixed(1)}%</Badge>
                  </div>
                  <span className="font-mono text-sm font-bold text-emerald-600 dark:text-emerald-400">@{item.odds?.toFixed(2)}</span>
                </div>
              ))}
            </div>
          </Card>
